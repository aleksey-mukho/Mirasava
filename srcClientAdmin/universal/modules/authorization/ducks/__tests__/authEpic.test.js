...
describe('authEpic', () => {
  ...
  test('Save tokens in cookies, when authApi.fetchTokens answered', () => {
    ...
    expect(dependencies.authApi.fetchTokens).toHaveBeenCalledWith(username, password);

    expect(authUtils.getStringifyAccessToken).toHaveBeenCalledTimes(2);
    expect(authUtils.getStringifyAccessToken).toHaveBeenCalledWith(TOKEN_ACCESS);

    expect(dependencies.cookies.set).toHaveBeenCalledTimes(2);
    expect(dependencies.cookies.set).toHaveBeenCalledWith(authTokensCookiesName, tokens);
  });
});
