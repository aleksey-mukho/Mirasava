...
class NotificationUI extends PureComponent<PropsType> {
  animateIsClosed = R.when(
    R.equals(this.props.id),
    R.always(s.animateClosed),
  )

  render() {
    const {
      errorType, title, message, id, closeNotification, idClosedNotification,
    } = this.props;

    return (
      <div className={
        styles([s.container, this.animateIsClosed(idClosedNotification)])}
      >
