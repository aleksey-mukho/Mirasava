...
describe('api/utils/index.makeRequest', () => {
  ...
  test('getTokensAndRequest was called, when isHaveTempRefreshTokenAndAccessTokenExpiredAndNeedToken returned true', (done) => {
    ...
    utils.isHaveTempTokensAndAccessTokenNotExpiredAndNeedToken = jest.fn().mockReturnValue(false);
    utils.isHaveTempRefreshTokenAndAccessTokenExpiredAndNeedToken = jest.fn().mockReturnValue(true);
    utils.makeRequest({
      ...
      .subscribe({
        next: (resp) => {
          ...
          expect(utils.getTokensAndRequest).toHaveBeenCalledWith({ cookies, config, body });
        },
        complete: done,
      });
  });
  ...
  test('request was called, when isHaveTempTokensAndAccessTokenNotExpiredAndNeedToken returned true', (done) => {
    ...
    utils.isHaveTempTokensAndAccessTokenNotExpiredAndNeedToken = jest.fn().mockReturnValue(true);
    utils.isHaveTempRefreshTokenAndAccessTokenExpiredAndNeedToken = jest.fn()
    ...
    utils.makeRequest({
      ...
      .subscribe({
        next: (resp) => {
          ...
          expect(utils.request).toHaveBeenCalledWith({ config, body, tokenAccess: TOKEN_ACCESS });
        },
        complete: done,
      });
  });
