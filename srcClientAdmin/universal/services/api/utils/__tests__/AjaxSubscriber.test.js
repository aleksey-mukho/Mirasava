...
describe('AjaxSubscriber', () => {
  describe('this.req.onload', () => {
    ...
    test('Return responce when this.req.status === 200',
      (done) => {
    ...
    test('Return responce when this.req.status === 201',
      (done) => {
        Observable.create((observer) => {
          const subscriber = new AjaxSubscriber({
            observer, config, body, tokenAccess,
    ...
    test('Return responce when this.req.status === 204',
      (done) => {
        Observable.create((observer) => {
          const subscriber = new AjaxSubscriber({
            observer, config, body, tokenAccess,
    ...
    test('Return error when this.req.status !== 200 || 201 || 204',
      (done) => {
        Observable.create((observer) => {
          const subscriber = new AjaxSubscriber({
            observer, config, body, tokenAccess,
  ...
  describe('settingXHRrequest', () => {
    ...
    test('setRequestHeader was called 2 times, when accessToken !== undefined', () => {
      ...
      expect(subscriber.req.open).toHaveBeenCalledWith(
        config.method, `${environment}/${config.url}`, true,
      );
