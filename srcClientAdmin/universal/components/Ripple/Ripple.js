...
class Ripple extends React.PureComponent<PropsType> {
  calculateRipplePosition = ({ event, parent, size }: {
    event: EventType,
  ...
  calculateRippleSize = ({ parent }: {
    ...
    R.compose(
      R.ifElse(
        ({ offsetWidth, offsetHeight }) => R.lte(
        ...
        (rippleSize) => {
          this.renderRipple({
            ...
            position: this.calculateRipplePosition({
              // $FlowFixMe
              event, parent: this.rippleContainer, size: rippleSize,
            }),
          });
        },
        // $FlowFixMe
  ...
  render() {
    const {
      children, className: rippleContainerClassName,
    } = this.props;
